...
export default function ChatOverlay({ onClose }) {
  ...
  return (
    ...
    >
      {/* Chat panel */}
      <div
        className="w-full max-w-md flex flex-col bg-white rounded-xl shadow-elevated border border-surface-200 overflow-hidden animate-slide-up"
        style={{ height: 'min(600px, calc(100vh - 40px))' }}
      >
        {/* ── Header ─────────────────────────────── */}
        <div className="flex items-center gap-2.5 px-4 py-3 border-b border-surface-200 bg-white">
          <div className="w-7 h-7 rounded-md bg-brand-600 flex items-center justify-center text-white flex-shrink-0">
            <Brain className="w-4 h-4" />
          </div>
          <div className="flex-1">
            <p className="font-bold text-slate-900 text-xs sm:text-sm tracking-tight">PathMind AI Advisor</p>
          ...
          <button
            onClick={onClose}
            className="btn-ghost p-1 text-text-muted hover:text-text-primary rounded-md"
        ...
        {/* ── Messages ───────────────────────────── */}
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3.5 bg-surface">
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-2.5 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {m.role === 'assistant' && (
                <div className="w-6 h-6 rounded bg-brand-100 flex items-center justify-center flex-shrink-0 mt-0.5 text-brand-700">
        ...
        {messages.length === 1 && !loading && (
          <div className="px-3.5 py-2.5 bg-white border-t border-surface-200">
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <MessageSquare className="w-3 h-3" /> Suggested Inquiries
            </p>
            <div className="flex flex-wrap gap-1.5">
              {SUGGESTIONS.map(s => (
                <button
                  ...
                  className="text-xs px-2.5 py-1 rounded bg-surface-100 text-slate-700 border border-surface-200 hover:bg-surface-200 hover:border-slate-300 transition-colors"
                >
                  {s}
                </button>
              ))}
